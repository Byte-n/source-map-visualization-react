...
- 🎯 **直观映射** - 可视化显示压缩代码与源代码之间的映射关系
- 🎨 **高度可定制** - 支持自定义样式、工具栏和渲染函数
- 📱 **响应式设计** - 自动适应容器大小变化
- 🔍 **交互式调试** - 支持点击跳转、悬停高亮等交互功能
- ⚡ **高性能** - 基于 Canvas 渲染，支持大型代码文件
...
| 属性名         | 类型                  | 必填 | 默认值             | 说明                      |
| -------------- | --------------------- | ---- | ------------------ | ------------------------- |
| `code`         | `string`              | ❌   | -                  | 压缩后的 JavaScript 代码。如果 Source Map 中包含 `sourcesContent`，则可以省略此参数 |
...
**参数：**
- `row: number` - 行号（1-based，从 1 开始）
- `col: number` - 列号（0-based，从 0 开始）
- `config?: { throwErrorOnOutOfRange?: boolean }` - 配置选项
  - `throwErrorOnOutOfRange` - 当行号超出范围时是否抛出错误，默认为 `false`

**功能：**
- 滚动并高亮压缩代码面板中指定位置的代码块
- 若存在映射则联动滚动到对应的源代码位置并绘制连线
- 当行号超出范围时，自动跳转到最后一行
- 高亮恢复受 `hoverRestoreDelayMs` 控制，默认 2000ms 后自动恢复
